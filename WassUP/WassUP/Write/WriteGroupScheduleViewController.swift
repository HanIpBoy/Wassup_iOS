...
class WriteGroupScheduleViewController: UIViewController {
    ...
    @IBAction func saveSchedule(_ sender: UIButton) { // 일정 저장
        ...
        if deleteButton.isHidden { // 생성
            ...
            server.postDataToServer(requestURL: "group/schedule", requestData: scheduleMap, token: UserDefaults.standard.string(forKey: "token")!) { (data, response, error) in
                if let error = error {
                    print("Error: \(error)")
                    return
                }
            ...
            server.updateData(requestURL: "group/schedule", requestData: updateMap, token: UserDefaults.standard.string(forKey: "token")!) { (data, response, error) in
                if let error = error {
                    print("Error: \(error)")
                    return
                }
...
extension WriteGroupScheduleViewController {
...
extension WriteGroupScheduleViewController: UIPickerViewDelegate, UIPickerViewDataSource {
